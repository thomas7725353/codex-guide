...
兜底 skill 保存在仓库的 `skills/codex-gorustai-bootstrap/SKILL.md`，会编译进 `codex-guide` 二进制，也会打进 Windows ZIP / macOS DMG。安装器会把它安装到用户本机的 `~/.agents/skills/codex-gorustai-bootstrap/SKILL.md`，同时写入 `${CODEX_HOME:-~/.codex}/skills/codex-gorustai-bootstrap/SKILL.md`，保证 Codex CLI 能发现兜底 skill。

中国用户也可以从 Worker 直接下载：

https://guide.gorustai.com/skills/codex-gorustai-bootstrap/SKILL.md

## 维护规则

每次修改安装脚本、Worker 镜像、文档、故障处理流程或 bug 修复时，都必须同步检查兜底 skill 是否需要更新。需要更新时，直接修改 `skills/codex-gorustai-bootstrap/SKILL.md`，确保二进制 embed、安装包文件和 Worker 下载入口都保持一致。
...
打开 DMG 后双击 `安装Codex.command`。DMG 内置对应架构的 Codex CLI 离线包，所以文件会比较大。
